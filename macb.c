...
#define TX_RING_SIZE		256 /* must be power of 2 */
...
#define MACB_TX_ERR_FLAGS	(MACB_BIT(ISR_TUND)			\
					| MACB_BIT(ISR_RLE)		\
					| MACB_BIT(TXERR))
#define MACB_TX_INT_FLAGS	(MACB_TX_ERR_FLAGS )
//| MACB_BIT(TCOMP)
...
static void macb_tx_non_interrupt(struct macb_queue *queue)
{
	unsigned int tail;
	unsigned int head;
	u32 status;
...
static void macb_tx_interrupt(struct macb_queue *queue)
{
	...
	struct macb *bp = queue->bp;
	u16 queue_index = queue - bp->queues;
//	printk(KERN_ERR "TX Interrupt triggered\n");
...
static void gem_rx_refill(struct macb *bp)
{
	...
	struct sk_buff		*skb;
	dma_addr_t		paddr;
	struct timespec startTs, endTs, endTs2;
...
memcpy(queue->tx_dma_mem[entry].dma_local_addr, skb->data + offset, size);


			tx_skb->skb = NULL;
			//		tx_skb->mapping = mapping;
			tx_skb->mapping = queue->tx_dma_mem[entry].dma_handle;
			tx_skb->size = size;
...
//			BDEBUG(getnstimeofday(&endTs2));

//			BDEBUG(bp->macbrx += as_nanoseconds(&endTs2) - as_nanoseconds(&startTs));
...
//			}
			...
			paddr = bp->rx_dma_mem[entry].dma_handle;
...
// new
		...
		memcpy(skb->data, bp->rx_dma_mem[entry].dma_local_addr+2, len);
// new
		skb_put(skb, len);
		addr = MACB_BF(RX_WADDR, MACB_BFEXT(RX_WADDR, addr));
//		printk(KERN_ERR "XLNX: Received %u\n",len);
//		print_hex_dump(KERN_ERR, "data: ", DUMP_PREFIX_ADDRESS, 16, 1, bp->rx_dma_mem[entry].dma_local_addr+2, len-2,1);
...
static irqreturn_t macb_interrupt(int irq, void *dev_id)
{
	...
	if (unlikely(!status))
		return IRQ_NONE;
//	if ((status & MACB_BIT(TCOMP)) == MACB_BIT(TCOMP)) {
//		printk(KERN_ERR "Transfer complete int\n");
//	}
	if ((status == MACB_BIT(TCOMP)) && (queue->please_clear == 0)){
		if (bp->caps & MACB_CAPS_ISR_CLEAR_ON_WRITE)	
			queue_writel(queue, ISR, MACB_BIT(TCOMP));		
		bp->txirq_quick_count++;

		BDEBUG(getnstimeofday(&endTs));	
		BDEBUG(bp->txirq_time += as_nanoseconds(&endTs) - as_nanoseconds(&startTs));	
		return IRQ_HANDLED;		
	}
	
/*	if ((status == MACB_BIT(TCOMP)) && (CIRC_SPACE(queue->tx_head, queue->tx_tail, TX_RING_SIZE) > 40)){
		if (bp->caps & MACB_CAPS_ISR_CLEAR_ON_WRITE)	
			queue_writel(queue, ISR, MACB_BIT(TCOMP));		
		bp->txirq_quick_count++;
...
//		bp->txirq_short_time += as_nanoseconds(&endTs) - as_nanoseconds(&startTs);	
		return IRQ_HANDLED;
	}*/
	spin_lock(&bp->lock);
				...
				struct sk_buff *skb)
{
	...
	u32 ctrl;
	struct timespec startTs, endTs;
	struct timespec startTs2, endTs2;
	void *frag_address = NULL;
	BDEBUG(getnstimeofday(&startTs));
...
//		tx_skb->mapping = mapping;
	...
	BDEBUG(getnstimeofday(&startTs2));
	...
	for (f = 0; f < nr_frags; f++) {
		...
		while (len) {
			...
			frag_address = skb_frag_address_safe(frag);
			
			if (frag_address) {
				memcpy(queue->tx_dma_mem[entry].dma_local_addr, frag_address, size); 
			} else {
				printk(KERN_ERR "XLNX: Unable to handle non-paged data\n");
				skb_copy_bits(skb, offset, queue->tx_dma_mem[entry].dma_local_addr ,size);
...
//	printk(KERN_ERR "XLNX: Packets in one go %i\n",count);
	BDEBUG(bp->txdma_time += as_nanoseconds(&endTs) - as_nanoseconds(&startTs));
    
	BDEBUG(bp->txqueue_time += as_nanoseconds(&endTs2) - as_nanoseconds(&startTs2));
...
static int macb_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	spin_lock_irqsave(&bp->lock, flags);
	BDEBUG(getnstimeofday(&endTs2));
	BDEBUG(bp->dbg_bs += as_nanoseconds(&endTs2) - as_nanoseconds(&startTs));
...
//			}
	...
	if (queue->please_clear == 1) {
//		printk(KERN_ERR "XLNX: Please clear reset\n");
		queue->please_clear=0;
	}

	/* Map socket buffer for DMA transfer */
	if (!macb_tx_map(bp, queue, skb)) {
		printk(KERN_ERR "XLNX: ARGHLKJ\n");
...
/*	if (CIRC_SPACE(queue->tx_head, queue->tx_tail, TX_RING_SIZE) < 1){
		macb_tx_non_interrupt(queue);
		if (CIRC_SPACE(queue->tx_head, queue->tx_tail, TX_RING_SIZE) < 1){
  			printk(KERN_ERR "XLNX: Forced into busy state at end\n");
...
unlock:
	spin_unlock_irqrestore(&bp->lock, flags);
		BDEBUG(getnstimeofday(&endTs));	
		BDEBUG(bp->txtotal_time += as_nanoseconds(&endTs) - as_nanoseconds(&startTs));	
...
static int macb_alloc_consistent(struct macb *bp)
{
	...
	for (q = 0, queue = bp->queues; q < bp->num_queues; ++q, ++queue) {
		...
		if (!queue->tx_skb)
			goto out_err;
		queue->tx_dma_mem = kzalloc(TX_RING_SIZE * sizeof(struct dma_prealloc), GFP_KERNEL);
		//
		if (!queue->tx_dma_mem){
			printk(KERN_ERR "XLNX: Couldn't allocate the memory to record the memory\n");
			goto out_err;
		}

		for (ii = 0; ii < TX_RING_SIZE; ii++){
			queue->tx_dma_mem[ii].dma_local_addr = dma_alloc_coherent(&bp->pdev->dev, 4096, &(queue->tx_dma_mem[ii].dma_handle), GFP_KERNEL | GFP_DMA);
			if (!queue->tx_dma_mem[ii].dma_local_addr) {
					printk(KERN_ERR "XLNX: Failed to allocate coherent buffer %i\n",ii);
			}
	...
	bp->rx_dma_mem = kzalloc(RX_RING_SIZE * sizeof(struct dma_prealloc), GFP_KERNEL);
	for (ii = 0; ii < RX_RING_SIZE; ii++) {
		bp->rx_dma_mem[ii].dma_local_addr = dma_alloc_coherent(&bp->pdev->dev, 4096, &(bp->rx_dma_mem[ii].dma_handle), GFP_KERNEL | GFP_DMA);
		if (!bp->rx_dma_mem[ii].dma_local_addr) {
			printk(KERN_ERR "XLNX: Failed to allocate RX coherent buffer %i\n", ii);
		}

	}

	printk(KERN_ERR "XLNX: Passed setup\n");
...
static void macb_set_rx_mode(struct net_device *dev)
{
	...
	printk(KERN_ERR "XLNX: stats time spend \nTX Time DMA part %llu\nTX Time Queue part %llu\nTX Total time %llu\nIRQ total time %llu\nIRQ shorted time %llu\nNumber of IRQs total %llu\nNumber of IRQs shorted %llu\nReceive process time %llu\nDEBUG binary search %llu\n",bp->txdma_time,bp->txqueue_time,bp->txtotal_time,bp->txirq_time,bp->txirq_short_time, bp->txirq_base_count, bp->txirq_quick_count, bp->macbrx,bp->dbg_bs);
